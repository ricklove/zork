export function boom_room() {
    ...
    let o: OBJECT = null;
    ((vname(prsact) === walk_in_X_words || (vname(prsact) === on_X_words && dummy_Q = t))) ? {
      return (((memq(o = find_obj(`CANDL`), aobjs(win)) && 1_Q(olight_Q(o))) || (memq(o = find_obj(`TORCH`), aobjs(win)) && 1_Q(olight_Q(o))))) ? {
          return unwind(prog(/*(*/ [] /*)*/,
		    (dummy_Q) ? {
...
export function bats_room() {
    let prsact: VERB = G_prsvec[1];
    ((vname(prsact) === walk_in_X_words && !memq(find_obj(`GARLI`), aobjs(G_winner)))) ? {
      return fly_me();
...
export function dumbwaiter() {
    ...
    (prsact === G_raise_X_words) ? {
      ...
      return (((ct && here === top) || (!ct && here === bot))) ? {
...
export function machine_function() {
    ...
    (G_here === find_room(`MACHI`)) ? {
      return (vname(prsact) === open_X_words) ? {
          return (oopen_Q(mach)) ? {
              return tell(pick_one(dummy));
            } : (tell(`The lid opens.`)) ? {
...
export function cliff_function() {
    (memq(find_obj(`RBOAT`), aobjs(G_winner))) ? {
...
export function stick_function() {
    let prsact: VERB = G_prsvec[1];
    (vname(prsact) === wave_X_words) ? {
      return ((G_here === find_room(`FALLS`) || G_here === find_room(`POG`))) ? {
          return (!G_rainbow_X_flag) ? {
...
export function rboat_function(arg?: (FALSE | ATOM)) {
    ...
    (arg) ? {
      ...
      return (memq(find_obj(`STICK`), aobjs(G_winner))) ? {
      ...
      return (avehicle(G_winner) === rboat) ? {
...
export function beach_room() {
    ...
    let here: ROOM = G_here;
    let cnt: FIX = null;
    ((vname(prsact) === dig_X_words && shov === G_prsvec[2])) ? {
...
export function tcave_room() {
    ...
    let here: ROOM = G_here;
    let cnt: FIX = null;
    ((vname(prsact) === dig_X_words && G_prsvec[2] === shov)) ? {
      return (memq(find_obj(`GUANO`), robjs(here))) ? {
          here[G_rvars] = cnt = _(1, rvars(here));
          return (cnt > 3) ? {
...
define(balloon, ballact, /*(*/ [`OPTIONAL`, /*(*/ [arg, false] /*)*/,
	...
	(arg === read_in) ? {
      return (prsa === G_walk_X_words) ? {
          return (m = memq(chtype(prsvec[2], atom),
					rexits(G_here))) ? {
              return (G_btie_X_flag) ? {
	...
	(prsa === G_burn_X_words) ? {
      return (memq(prso,ocontents(cont))) ? {
          tell(`The `,
			    1,
			    odesc2(prso),
      ...
      return (((oopen_Q(cont) && G_binf_X_flag) || spname(rid(G_here))[`LEDG`])) ? {
...
export function wire_function() {
    ...
    (prsa === G_tie_X_words) ? {
      return ((prso === find_obj(`BROPE`) && (prsi === find_obj(`HOOK1`) || prsi === find_obj(`HOOK2`)))) ? {
          G_btie_X_flag = t;
          clock_disable(bint);
    ...
    (prsa === G_burn_X_words) ? {
      ...
      return (ocan(fuse) === brick) ? {
          ...
          return (brick_room === G_here) ? {
              ...
              return (memq(brick,ocontents(find_obj(`SSLOT`)))) ? {
              ...
              mapf(false,
				   ...
				   robjs(brick_room));
              return (brick_room === find_room(`LROOM`)) ? {
                  mapf(false,
					  function(x: OBJECT) {
                        x[G_ocan] = false;
                      },
...
export function safe_mung() {
    ...
    (G_here === rm) ? {
      return jigs_up((rtrnn(rm,G_rhousebit)) ? {
...
to the bottom.`, 1,
		(rtrnn(find_room(`SAFE`), G_rmungbit)) ? {
...
export function gnome_function() {
    ...
    let prsa: VERB = pv[1];
    let prso: PRSOBJ = pv[2];
    (((prsa === G_give_X_words || prsa === G_throw_X_words) && type_Q(prso,object) && (otval(prso) !== 0) ? {
